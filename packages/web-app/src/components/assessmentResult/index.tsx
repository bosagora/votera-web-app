import {
  ...
  const AssessmentResult: React.FC<Props> = ({values}) => {
    ...
    useEffect(() => {
      if (values?.length > 0) {
        const totalAverage = values.reduce((sum, assessment) => 
          sum + calculateAssessmentAverage(assessment), 0);
    ...
    const calculateItemAverage = (key: keyof Assessment) => {
      return values.reduce((sum, assessment) => sum + assessment[key], 0) / values.length;
    };
      ...
      <>
        <Container>
          <InputWrapper>
            ...
            <NodeInfoWrapper>참여한 노드 {values.length}</NodeInfoWrapper>
          </InputWrapper>
            ...
            {assessmentTypes.map(({key, label}) => (
              <ProgressInfoWrapper key={key}>
                ...
                <LinearProgressContainer>
                  <LinearProgress 
                    max={max} 
                    value={calculateItemAverage(key)} 
                  />
                  <ProgressInfo>
                    <ApprovalAddresses
                      style={{
  ...
  const ProgressWrapper = styled.div.attrs({
    className: 'flex flex-1 flex-col items-stretch w-full gap-y-4 order-2 desktop:order-none'
  })``;
