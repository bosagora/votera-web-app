...
async function main() {
    ...
    for (let idx = 0; idx < voterLengthOfManager; idx += size) {
        for await (const step of await client.methods.createParticipantPart(
            proposalId,
            idx,
            Math.min(idx + size, voterLengthOfManager)
        )) {
            switch (step.key) {
                ...
                case NormalSteps.SENT:
                    console.log(`createParticipantPart: ${page + 1}/${maxPage} - ${step.txHash}...`);
                    expect(step.proposalId).equal(proposalId);
